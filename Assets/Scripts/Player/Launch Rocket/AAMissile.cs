...
public class AAMissile : MonoBehaviour
{
    ...
    private void Start()
    {
        player = FindObjectOfType<Aircraft>();
        //initialSpeed = 1.5f * player.rb.velocity.magnitude;
        initialSpeed = player.rb.velocity.magnitude;
        timeToLive = Random.Range(8,20);
    ...
    private void FixedUpdate()
    {
        ...
        if(target != null)
        {
            ...
            //{
            //    if(Gmanager.insatce.rocketDanger != null)
            //    if (!Gmanager.insatce.rocketDanger.activeSelf)
            //    {
            //        if (Vector3.Distance(target.position, transform.position) < 500)
            //        {
            //            Gmanager.insatce.rocketDanger.SetActive(true);
            //            //Gmanager.insatce.HiderocketDanger();
            //        }
            //    }
            //    if (Vector3.Distance(target.position,transform.position) < 5)
            //    {
                    
            //        if (target.gameObject.GetComponent<Health>() != null && !missileContact)
            //        {
            //            target.gameObject.GetComponent<Health>().health -= 1;
            //            missileContact = true;
            //        }
            //        exploSound.Play();
            //        DestroyMissile(true);
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (isLaunched && TimeSince(launchTime) > dropDelay)
        {
            ...
            collision.gameObject.GetComponent<DamageManager>().HP = 0;
